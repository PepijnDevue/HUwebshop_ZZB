...
def sessions_to_postgre(cursor, sessions, connection):
    """
    The function sessions_to_postgre() is here to insert all the session information gatherd from mongo into the postgres database.

    Parameters:
        cursor: The psygopg2 cursor is used to execute sql querys.
        sessions: This is a list full of dictonarys with all the information about the sessions that we want to insert into the postgres database.
        connection: The connection is used to commmit the changes add the end of all the querys being made in the cursor.
